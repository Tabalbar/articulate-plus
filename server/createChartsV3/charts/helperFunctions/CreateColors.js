...
module.exports = (header, data) => {
  ...
  colorGradient.setNumberRange(1, unique.length);
  let startColor = "red";
  let endColor = "blue";
  for (let i = 0; i < covidColors.length; i++) {
    if (covidColors[i].header == header) {
      startColor = covidColors[i].startColor;
      endColor = covidColors[i].endColor;
      break;
    }
  }
  colorGradient.setSpectrum(startColor, endColor);
  for (let i = 0; i < unique.length; i++) {
    colorArray.push("#" + colorGradient.colourAt(i));
  }
