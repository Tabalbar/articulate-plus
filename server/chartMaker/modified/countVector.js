...
module.exports = (transcript, headerMatrix, synonymAttributes, data, modifiedChartOptions) => {
    let headers = []
    ...
    if(modifiedChartOptions.semanticAnalysis && modifiedChartOptions.window.toggle){
        ...
        sentences.slice(Math.max(sentences.length - modifiedChartOptions.window.pastSenteces, 0))
        for(let i = 0; i < sentences.length; i ++) {
            let sentence = nlp(sentences[i])
            sentence.nouns().toSingular()
