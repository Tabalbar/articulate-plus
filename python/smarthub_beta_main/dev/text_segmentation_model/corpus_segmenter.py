...
class CorpusSegmenter(Segmenter):
    def __init__(self):
        super().__init__()

    def get_segments_by_subject(self):
        ...
        for subject in ['subject' + str(i) for i in range(5, 21)]:
            ...
            #if subject == 'subject16':
            #    print("HEY")

            # get all the transcripts for current subject excluding those utterances with 3 words or less
            #transcript_utterances = []
            #for utterance_gens in transcripted_utterances_by_subject[subject]:
            #    transcript_utterances = [(line_id, utterance.text, utterance) for line_id, utterance in
            #                             enumerate(utterance_gens) \
