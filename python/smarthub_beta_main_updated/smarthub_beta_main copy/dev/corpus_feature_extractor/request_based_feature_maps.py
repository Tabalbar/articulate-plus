...
class RequestBasedFeatureMaps(FeatureExtractor):
        ...
        for subject_name, subject_tokens in corpus:
            ...
            for subject_token in subject_tokens:
                ...
                for utterance in tokenized_requests:
                    if tokenize_as_text:
                        context.append(tokenized_split.join([token.text for token in utterance]))
                    else:
    ...
    def create_features_vector(
        ...
        if index > 0 and include_surrounding:
            ...
            else:            # return [self.non_entity_nlp(utterance) for utterance in utterances]
                X_utt_length = None
        ...
        if index < len(utterances) - 1 and include_surrounding:
            ...
            if include_avg_word_embeddings:
                X_we = self.get_average_word_embeddings(utterances=[next_utterance])
            else:
                X_we = None
            if include_sentiment:
                X_sentiment = self.get_average_sentiment(utterances=[next_utterance])
            else:
                X_sentiment = None
            if include_utt_length:
                X_utt_length = self.get_average_utt_length(utterances=[next_utterance])
            else:
                X_utt_length = None
            if include_number_of_slots:
                X_number_of_slots = self.get_average_number_of_slots(utterances=[next_utterance])
            else:
                X_number_of_slots = None
            if include_number_of_non_modal_verbs:
                X_number_of_non_modal_verbs = self.get_average_number_of_non_modal_verbs(utterances=[next_utterance])
            else:
                X_number_of_non_modal_verbs = None
            if include_number_of_wh_words:
                X_number_of_wh_words = self.get_average_number_of_wh_words(utterances=[next_utterance])
            else:
                X_number_of_wh_words = None
